...
```{r}
meta_data <- read_excel("META-ANALYSIS-21-12-2021.xlsx")
...
subset.final$Author_Year <-paste(subset.final$Author_Year, subset.final$Pub_Year)
...
Physical Function
```{r}
forest.REI.phys<-subset(REI.phys, select = c(ID,Author_Year,Hedge.g,LL.g,UL.g))
forest.REI.phys<-forest.REI.phys[order(forest.REI.phys$ID),]
Overall.effect <-c(NA, "Overall Effect", 0.580, 0.328, 0.833)
...
forest.REI.phys.plot <- ggplot(forest.REI.phys, aes(y= ES, x = Hedge.g, xmin = LL.g, xmax= UL.g)) + geom_point() + geom_errorbarh(height=.1) +
  scale_y_continuous(name= "Author & Year", breaks=rev(1:nrow(forest.REI.phys)), labels=forest.REI.phys$Author_Year) +
...
Cognitive Function
```{r}
forest.REI.cog<-subset(REI.cog, select = c(ID,Author_Year,Hedge.g,LL.g,UL.g))
forest.REI.cog<-forest.REI.cog[order(forest.REI.cog$ID),]
Overall.effect <-c(NA, "Overall Effect", 0.508, 0.0137, 1.000)
...
forest.REI.cog.plot <- ggplot(forest.REI.cog, aes(y= ES, x = Hedge.g, xmin = LL.g, xmax= UL.g)) + geom_point() + geom_errorbarh(height=.1) +
  scale_y_continuous(name= "Author & Year", breaks=rev(1:nrow(forest.REI.cog)), labels=forest.REI.cog$Author_Year) +
  scale_x_continuous(name="Hedge's g Effect Size", limits = c(-3,3), breaks = c(-3, 0, 3)) + 
...
Falls Risk
```{r}
forest.REI.falls<-subset(REI.falls, select = c(ID,Author_Year,Hedge.g,LL.g,UL.g))
forest.REI.falls<-forest.REI.falls[order(forest.REI.falls$ID),]
Overall.effect <-c(NA, "Overall Effect", 0.448, -0.184, 1.080)
forest.REI.falls<-rbind(forest.REI.falls,Overall.effect)
forest.REI.falls$ES<- rev(c(1:5))
...
forest.REI.falls.plot <- ggplot(forest.REI.falls, aes(y= ES, x = Hedge.g, xmin = LL.g, xmax= UL.g)) + geom_point() + geom_errorbarh(height=.1) +
  scale_y_continuous(name= "Author & Year", breaks=rev(1:nrow(forest.REI.falls)), labels=forest.REI.falls$Author_Year) +
...
Quality of Life
```{r}
forest.REI.QOL<-subset(REI.QOL, select = c(ID,Author_Year,Hedge.g,LL.g,UL.g))
forest.REI.QOL<-forest.REI.QOL[order(forest.REI.falls$ID),]
Overall.effect <-c(NA, "Overall Effect", 0.919, -0.263, 2.100)
forest.REI.QOL<-rbind(forest.REI.QOL,Overall.effect)
forest.REI.QOL<- forest.REI.QOL[-c(4,5),]
...
forest.REI.QOL.plot <- ggplot(forest.REI.QOL, aes(y= ES, x = Hedge.g, xmin = LL.g, xmax= UL.g)) + geom_point() + geom_errorbarh(height=.1) +
  scale_y_continuous(name= "Author & Year", breaks=rev(1:nrow(forest.REI.QOL)), labels=forest.REI.QOL$Author_Year) +
  scale_x_continuous(name="Hedge's g Effect Size", limits = c(-3,3), breaks = c(-3, 0, 3)) + 
